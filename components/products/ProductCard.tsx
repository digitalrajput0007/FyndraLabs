...
import { ArrowRight, Receipt, Box } from "lucide-react";
...
export function ProductCard({ product, featured = false }: ProductCardProps) {
  ...
  return (
    ...
    >
      <div>
        {/* Top bar: Category (12-13px) + Status */}
        <div className="flex items-center justify-between gap-2 mb-6">
          <Badge variant="primary" className="text-xs sm:text-[13px] px-3 py-1">
            {product.category}
          </Badge>
          <Badge
            variant={product.status === "Active" ? "success" : "secondary"}
            className="text-xs sm:text-[13px] px-3 py-1"
          >
            {product.status}
          </Badge>
        </div>
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <h3 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white tracking-tight group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">
              {product.name}
            </h3>
        ...
        {/* Tagline */}
        <p className="text-slate-800 dark:text-slate-200 font-semibold text-base sm:text-[17px] mb-2.5">
          {product.tagline}
        </p>

        {/* Description (15-16px) */}
        <p className="text-slate-600 dark:text-slate-400 text-sm sm:text-[16px] leading-relaxed mb-6">
          {product.description}
        </p>
      </div>
      ...
      <div className="pt-4 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between gap-3">
        <Link
          ...
          className="inline-flex items-center gap-2 text-sm sm:text-[15px] font-semibold text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300 transition-colors group/link"
        >
