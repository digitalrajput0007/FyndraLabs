      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 lg:py-14">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 lg:gap-12">
          ...
          <div>
            ...
            {/* Social Links (only rendered if non-empty URL) */}
            {activeSocials.length > 0 && (
              <div>
                <h4 className="text-xs font-semibold uppercase tracking-wider text-slate-900 dark:text-slate-200 mb-2">
                  Connect
                </h4>
                <div className="flex flex-wrap gap-2">
                  {activeSocials.map(([platform, url]) => (
                    <a
                      ...
                      className="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white bg-slate-200/50 dark:bg-slate-800/50 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-md transition-colors capitalize"
                    >
                      <span>{platform}</span>
        ...
        {/* Bottom Copyright */}
        <div className="mt-10 pt-6 border-t border-slate-200/80 dark:border-slate-800/80 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 dark:text-slate-500">
